...
load_config(){ local root x; root="${GITHUB_ARTIFACTS_HOME:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.."&&pwd)}"; CONFIG_FILE="$root/configs/${1%.conf}.conf"; [[ -f "$CONFIG_FILE" ]]||blocked "Не найдена карточка публикации $1." "Попросите агента создать карточку."; source "$CONFIG_FILE"; for x in REPOSITORY ISSUE_NUMBER BASE_BRANCH WORK_BRANCH SOURCE_PATTERN TARGET_PATH FILE_TYPE EXPECTED_FILES MERGE_ALLOWED CLOSE_ISSUE_ALLOWED; do eval '[[ -n "${'"$x"':-}" ]]' || blocked "В карточке не заполнено поле $x." "Попросите агента исправить карточку."; done; REPO_NAME="${REPOSITORY#*/}"; }
...
check_expected_files_local(){ local i; local -a a; IFS=',' read -ra a<<<"$EXPECTED_FILES"; for i in "${a[@]}"; do [[ -f "$1/${i# }" ]]||blocked "В пакете отсутствует файл: ${i# }" "Добавьте полный состав пакета."; done; }
