...
load_config "$1"; need_cmd git; need_cmd shasum; need_cmd python3; ensure_gh_auth; resolve_source; validate_artifact "$SOURCE_FILE" "$FILE_TYPE"
...
for attempt in 1 2 3 4 5; do
  if gh api "repos/$REPOSITORY/git/blobs/$BLOB" --jq .content 2>>"$LOG_FILE" | python3 -c 'import sys,base64; sys.stdout.buffer.write(base64.b64decode(sys.stdin.read()))' >"$TMP" 2>>"$LOG_FILE"; then
    DOWNLOAD_OK=true
    break
  fi
  sleep "$attempt"
done
[[ "$DOWNLOAD_OK" == true ]]||blocked "Не удалось скачать Git blob из exact head SHA." "Повторите публикацию; PR уже создан."
cmp -s "$REPO_DIR/$TARGET_PATH" "$TMP"||blocked "Побайтовая проверка не пройдена." "Повторите публикацию."
[[ "$S256" == "$(sha256_file "$TMP")" ]]||blocked "SHA-256 не совпадает." "Повторите публикацию."
[[ "$BLOB" == "$(blob_sha_file "$TMP")" ]]||blocked "Git blob SHA не совпадает." "Повторите публикацию."
