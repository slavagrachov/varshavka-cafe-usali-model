...
if [[ "${1:-}" == --self-test ]]; then need_cmd git; need_cmd shasum; need_cmd python3; printf 'VERSION=%s\n' "$PROGRAM_VERSION"; pass; exit 0; fi
...
REPO_DIR=$(repo_root); require_clean_tree "$REPO_DIR"; git -C "$REPO_DIR" fetch origin "$BASE_BRANCH" >&3 2>&1; git -C "$REPO_DIR" checkout "$BASE_BRANCH" >&3 2>&1; git -C "$REPO_DIR" reset --hard "origin/$BASE_BRANCH" >&3 2>&1
if git -C "$REPO_DIR" show-ref --verify --quiet "refs/heads/$WORK_BRANCH"; then git -C "$REPO_DIR" checkout "$WORK_BRANCH" >&3 2>&1; git -C "$REPO_DIR" rebase "origin/$BASE_BRANCH" >&3 2>&1||blocked "Рабочую ветку нельзя обновить." "Разрешите конфликт ветки."; else git -C "$REPO_DIR" checkout -b "$WORK_BRANCH" >&3 2>&1; fi
...
git -C "$REPO_DIR" add -- "$TARGET_PATH" ${ADDITIONAL_PATHS:-}; git -C "$REPO_DIR" diff --cached --quiet||git -C "$REPO_DIR" commit -m "GOV-GITHUB-001: publish artifact for issue #$ISSUE_NUMBER" >&3 2>&1; git -C "$REPO_DIR" push -u origin "$WORK_BRANCH" >&3 2>&1
