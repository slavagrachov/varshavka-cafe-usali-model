...
# Короткие команды запускают целевые программы через bash.
BIN="$HOME/bin"
mkdir -p "$BIN"
...
create_wrapper() {
  ...
  # Удаляем старую символическую ссылку или файл до записи. Иначе перенаправление
  # через существующую symlink может перезаписать целевой скрипт репозитория и
  # создать бесконечный самовызов.
  rm -f "$wrapper"
