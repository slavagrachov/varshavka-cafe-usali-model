...
const dishes = [
  ...
  ['VKM-010','Микс-салат с креветками и яблоком','Салаты','cold',['CRUSTACEAN']],
  ...
  ['VKM-017','Ростбиф с луком и гренкой','Холодные закуски','cold_meat',['GLUTEN']],
  ['VKM-018','Традиционный красный борщ','Супы','hot',[]],
  ...
  ['VKM-024','Рис жасмин','Гарниры','hot',[]],
...
const cards = dishes.map(([code,name,section,type,present],i) => ({
  safety_card_id:`FSC-${String(i+1).padStart(3,'0')}`, dish_code:code, dish_name:name, section, version:'0.1.0', as_of_date:date,
  source_recipe_version:null,
  ...
  veto_reason:'No accepted recipe/SKU allergen dossier and no validated dish-specific critical limits, shelf life or site HACCP/PPK record',
  next_action:'Chef submits frozen recipe; Procurement submits SKU dossier; PPK owner validates hazards/limits and traceability before Gate B readiness review',
...
const allergenRows = dishes.map(([code,name,section,type,present]) => {
  const row = {dish_code:code,dish_name:name,version:'0.1.0',as_of_date:date};
  for (const a of allergens) row[a] = present.includes(a) ? 'PRESENT_DRAFT_NAME_BASED' : 'UNKNOWN_RECIPE_SKU';
  Object.assign(row,{
    matrix_status:'DRAFT_BLOCKED_PENDING_VALIDATION', evidence_ids:'EVD-0012;EVD-FS-002',
    method:'Name/category screening only; not a declaration of composition or absence',
...
for (const [code,name,section,type,present] of dishes) {
  const common = {dish_code:code,dish_name:name,version:'0.1.0',as_of_date:date};
  ...
  controls.push(
    ...
    {...common,control_id:`CCP-${code}-04`,stage:'STORAGE_HOLDING_SERVICE',control_class:'OPRP_CANDIDATE',hazard:'microbial growth/toxin formation or quality/safety loss during storage, holding, delivery or service',control_parameter:'manufacturer/validated storage conditions, time and shelf life by final process/channel',critical_limit:null,monitoring:'batch label and storage/holding record; exact method pending validation',corrective_action:'block release; isolate expired/untraceable/deviating product; documented disposal or justified disposition',evidence_ids:storageEvidence,status:'BLOCKED_PENDING_VALIDATION',blocker:`SBL-${code}-04`,owner:'Food Safety/PPK owner; Operations'},
...
const blockers = [];
for (const [code,name,section,type,present] of dishes) {
  const base = {dish_code:code,dish_name:name,as_of_date:date,status:'OPEN',dish_readiness_veto:'BLOCK'};
  const specific = productEvidence(type,present,name);
  ...
  blockers.push(
    ...
    {...base,blocker_id:`SBL-${code}-02`,severity:'S1_CRITICAL',parameter:'allergens_and_cross_contact',reason:'No frozen recipe, compound-ingredient composition or site cross-contact validation',missing_input:'chef-frozen recipe; SKU allergen declarations; shared equipment/flow map; cleaning validation',impact:'cannot provide complete allergen information or declare absence',owner:'Chef; Procurement; Food Safety/PPK owner',next_action:'complete recipe-to-SKU allergen review and validate cross-contact controls',checkpoint:'Gate B / before consumer offering',evidence_ids:'EVD-0012;EVD-FS-002'},
    ...
    {...base,blocker_id:`SBL-${code}-04`,severity:'S1_CRITICAL',parameter:'storage_cooling_reheating_shelf_life',reason:'No supported dish-specific storage, cooling, reheating, holding or realization regime',missing_input:'manufacturer conditions; site process; official applicability review; validation/testing where required',impact:'production planning, labels, delivery and service release are blocked',owner:'Food Safety/PPK owner; Operations',next_action:'establish and validate regimes without extrapolating unsupported values',checkpoint:'Gate B / Owner Gate',evidence_ids:storageEvidence},
...
console.log(JSON.stringify({cards:cards.length,allergen_rows:allergenRows.length,controls:controls.length,blockers:blockers.length,sources:sources.length},null,2));
